"""API integration tests using the FastAPI TestClient.
...
@pytest.fixture
def client():
    """Authenticated app client with a clean schema per test.

    Each test starts from an empty database, then registers a user so the
    session cookie is present for all app endpoints. LLM is patched per-test
    via _use_fake_llm.
    """
